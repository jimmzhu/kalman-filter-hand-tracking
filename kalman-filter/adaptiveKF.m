...
accel_vec = -1e4:.25e4:1e4;

% for deltIdx = 1:numel(accel_vec)
...
%state vector
for i = 2:numel(px)
    vel_tx(2) = (px(i) - px(i-1))/delta_t;
    vel_ty(2) = (py(i) - py(i-1))/delta_t;
    accel_tx = (vel_tx(2) - vel_tx(1))/delta_t;
    accel_savex(end+1) = accel_tx;
    accel_ty = (vel_ty(2) - vel_ty(1))/delta_t;
...
% 
% normx(deltIdx) = norm(px - stateSaves(:,1));
% normy(deltIdx) = norm(py - stateSaves(:,2));
% normxy(deltIdx) = norm([px;py] - [stateSaves(:,1);stateSaves(:,2)])
% 
% end
...
title('(Adaptive) Kalman Filter Estimates and Ground Truths');
xlabel('X');
ylabel('Y');
